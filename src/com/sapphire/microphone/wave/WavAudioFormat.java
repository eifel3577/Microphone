...
public class WavAudioFormat extends PcmAudioFormat {
	...
	private WavAudioFormat(int sampleRate, int sampleSizeInBits, int channels,
			boolean signed) {
	...
	public static class Builder {
		...
		public WavAudioFormat build() {
			if (_sampleSizeInBits == 8)
				return new WavAudioFormat(_sampleRate, _sampleSizeInBits,
						_channels, false);
			else
				return new WavAudioFormat(_sampleRate, _sampleSizeInBits,
						_channels, true);
	...
	/**
	 * Generates audio format data for Wav audio format. returning PCM format is
	 * little endian.
	 * 
	 * @param sampleRate
	 *            sample rate
	 * @param sampleSizeInBits
	 *            bit amount per sample
	 * @param channels
	 *            channel count. can be 1 or 2
	 * @return a RawAudioFormat suitable for wav format.
	 */
	public static WavAudioFormat wavFormat(int sampleRate,
			int sampleSizeInBits, int channels) {
		if (sampleSizeInBits == 8)
			return new WavAudioFormat(sampleRate, sampleSizeInBits, channels,
					false);
		else
			return new WavAudioFormat(sampleRate, sampleSizeInBits, channels,
					true);
